...
const productVariantZodSchema = z.object({
  label: z
    .string()
    .min(1, { message: 'Variant label is required (e.g. 150ml, 50g)' }),
  stock: z.coerce
...
const createProductZodSchema = z.object({
  body: z
    ...
    .refine(
      data =>
        data.compareAtPrice ? data.compareAtPrice > data.basePrice : true,
      {
...
const updateProductZodSchema = z.object({
  body: z
    ...
    .transform(data =>
      data.title ? { ...data, slug: createSlug(data.title) } : data,
    ),
});
